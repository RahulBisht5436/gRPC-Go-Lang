...
```go
module example.com/bidirectional
...
| Folder | Import path |
|---|---|
| `proto/` | `example.com/bidirectional/proto` |
| `server/` | `example.com/bidirectional/server` |
| `client/` | `example.com/bidirectional/client` |

Yaad karo `server/main.go` me:

```go
import pb "example.com/bidirectional/proto"
//        ^^^^^^^^^^^^^^^^^^^^^^^^ ye module name
//                                ^^^^^^^ + folder path
```

Aur `bideirectionalStreams.proto` me:
...
#### `bidirectional` — clean module name

Iss project me tumne `bidirectional` (single word, lowercase) use kiya — **bilkul Go convention ke according**. Pichle `clientStream` me camelCase tha jo unusual hai. Yaha clean hai.
...
Use hota hai. `example.com` learning/example me chalta hai.
...
|---|---|
| `github.com/golang/protobuf` | Legacy protobuf API. **Tumhare current source code me directly use nahi ho raha** — `go mod tidy` chalao to indirect ho jaayega. |
| `google.golang.org/grpc` | `import "google.golang.org/grpc"` server/client me |
| `google.golang.org/protobuf` | Generated code me indirectly required |

> **`golang/protobuf` vs `google.golang.org/protobuf`** — confusing similar names hain. Pehla **purana** (v1) API hai, doosra **naya** (v2). Modern code me sirf `google.golang.org/protobuf` chahiye. Agar tum `go mod tidy` chalao to galat wala automatic remove ya indirect ho jaayega.
...
| `google.golang.org/genproto/googleapis/rpc` | shared proto types (`status.proto`, `codes`) |
...
|---|---|
| `go mod init example.com/bidirectional` | Naya `go.mod` banata hai |
...
```powershell
cd "gRPC project\Greet Request and Response Streams"
go mod tidy
```

Ye command:

1. **`github.com/golang/protobuf` ko remove kar dega** direct se (kyunki tumhare code me use nahi hota).
...
| Cheez | Matlab |
|---|---|
| `module example.com/bidirectional` | Project ka naam, import paths ka root |
